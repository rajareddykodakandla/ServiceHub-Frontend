...
const LoginPage = () => {
  ...
  const fetchGoogleSuggestions = (type, value) => {
    ...
    autocompleteService.current.getPlacePredictions(options, (predictions, status) => {
      ...
      const suggestions = predictions.map(p => ({ description: p.description, place_id: p.place_id }));
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5000/servicehub/login', {
        email,
        password
      });
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/servicehub/register', payload);
                ...
                {addressSuggestions.length > 0 && (
                  <div className="bg-white border rounded shadow max-h-40 overflow-y-auto">
                    {addressSuggestions.map((s, i) => (
                      <div key={i} className="px-2 py-1 hover:bg-blue-100 cursor-pointer" onClick={() => handleAddressSelect(s)}>
                        {s.description}
                      </div>
                    ))}
                  </div>
                ...
                {citySuggestions.length > 0 && (
                  <div className="bg-white border rounded shadow max-h-40 overflow-y-auto">
                    {citySuggestions.map((s, i) => (
                      <div key={i} className="px-2 py-1 hover:bg-blue-100 cursor-pointer" onClick={() => { setCity(s.description); setCitySuggestions([]); }}>
                        {s.description}
                      </div>
                    ))}
                  </div>
                ...
                {stateSuggestions.length > 0 && (
                  <div className="bg-white border rounded shadow max-h-40 overflow-y-auto">
                    {stateSuggestions.map((s, i) => (
                      <div key={i} className="px-2 py-1 hover:bg-blue-100 cursor-pointer" onClick={() => { setState(s.description); setStateSuggestions([]); }}>
                        {s.description}
                      </div>
                    ))}
                  </div>
                ...
                {zipSuggestions.length > 0 && (
                  <div className="bg-white border rounded shadow max-h-40 overflow-y-auto">
                    {zipSuggestions.map((s, i) => (
                      <div key={i} className="px-2 py-1 hover:bg-blue-100 cursor-pointer" onClick={() => { setZipCode(s.description); setZipSuggestions([]); }}>
                        {s.description}
                      </div>
                    ))}
                  </div>
